...
if SEI_con == 1:
    data = scipy.io.loadmat("Cycler_Output_KL_ageing.mat")
if SEI_con == 2:
    data = scipy.io.loadmat("Cycler_Output_KL_ageing.mat")
...
output_j_SEI = {"time (s)": time_vector_model_1,
  "j_SEI_0 (mols/m^3)": j_SEI_n_data_model_1[0,:],
  "j_SEI_n_s (mols/m^3)":j_SEI_n_data_model_1[-1,:]}

output_c_SEI = {"time (s)": time_vector_model_1,
  "c_SEI_0 (mols/m^3)": c_SEI_n_data_model_1[0,:],
  "c_SEI_n_s (mols/m^3)":c_SEI_n_data_model_1[-1,:]}

output_L_SEI = {"time (s)": time_vector_model_1,
  "L_SEI_0 (mols/m^3)": L_SEI_n_data_model_1[0,:],
  "L_SEI_n_s (mols/m^3)":L_SEI_n_data_model_1[-1,:]}

output_R_SEI = {"time (s)": time_vector_model_1,
  "R_SEI_0 (mols/m^3)": R_SEI_n_data_model_1[0,:],
  "R_SEI_n_s (mols/m^3)":R_SEI_n_data_model_1[-1,:]}

output_eps_s_n = {"time (s)": time_vector_model_1,
  "eps_s_n_0 (mols/m^3)": eps_e_n_model_1[0,:],
  "eps_s_n_n_s (mols/m^3)":eps_e_n_model_1[-1,:]}
...
if SEI_con == 1:
    ...
    df.to_csv('Pybamm_custom_CCCV_voltage_KL_ageing.csv')

    #load output data into a DataFrame object:
    df = pd.DataFrame(output_cell_current)

    # writing output data frame to a CSV file
    df.to_csv('Pybamm_custom_CCCV_current_KL_ageing.csv')

    #load output data into a DataFrame object:
    df = pd.DataFrame(output_j_SEI)

    # writing output data frame to a CSV file
    df.to_csv('Pybamm_custom_CCCV_j_SEI_KL_ageing.csv')

    #load output data into a DataFrame object:
    df = pd.DataFrame(output_c_SEI)

    # writing output data frame to a CSV file
    df.to_csv('Pybamm_custom_CCCV_c_SEI_KL_ageing.csv')

    #load output data into a DataFrame object:
    df = pd.DataFrame(output_L_SEI)

    # writing output data frame to a CSV file
    df.to_csv('Pybamm_custom_CCCV_L_SEI_KL_ageing.csv')

    #load output data into a DataFrame object:
    df = pd.DataFrame(output_R_SEI)

    # writing output data frame to a CSV file
    df.to_csv('Pybamm_custom_CCCV_R_SEI_KL_ageing.csv')


    #load output data into a DataFrame object:
    df = pd.DataFrame(output_eps_s_n)

    # writing output data frame to a CSV file
    df.to_csv('Pybamm_custom_CCCV_eps_e_n_KL_ageing.csv')
...
if SEI_con == 2:
    ...
    # writing output data frame to a CSV file
    df.to_csv('Pybamm_custom_CCCV_voltage_DL_ageing.csv')

    #load output data into a DataFrame object:
    df = pd.DataFrame(output_cell_current)

    # writing output data frame to a CSV file
    df.to_csv('Pybamm_custom_CCCV_current_DL_ageing.csv')

    #load output data into a DataFrame object:
    df = pd.DataFrame(output_j_SEI)

    # writing output data frame to a CSV file
    df.to_csv('Pybamm_custom_CCCV_j_SEI_DL_ageing.csv')

    #load output data into a DataFrame object:
    df = pd.DataFrame(output_c_SEI)

    # writing output data frame to a CSV file
    df.to_csv('Pybamm_custom_CCCV_c_SEI_DL_ageing.csv')

    #load output data into a DataFrame object:
    df = pd.DataFrame(output_L_SEI)

    # writing output data frame to a CSV file
    df.to_csv('Pybamm_custom_CCCV_L_SEI_DL_ageing.csv')

    #load output data into a DataFrame object:
    df = pd.DataFrame(output_R_SEI)

    # writing output data frame to a CSV file
    df.to_csv('Pybamm_custom_CCCV_R_SEI_DL_ageing.csv')
    ...
    # writing output data frame to a CSV file
    df.to_csv('Pybamm_custom_CCCV_eps_e_n_DL_ageing.csv')
