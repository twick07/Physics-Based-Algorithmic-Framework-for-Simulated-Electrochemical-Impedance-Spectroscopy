%% Model Parameters for MATLAB Scripts
...
if param == 1
    ...
    %L_SEI =  1e-8;%%"Initial SEI Thickness [m]"
    %L_SEI =  5e-11;%%"Initial SEI Thickness [m]"
    L_SEI = 1e-9;
    %L_SEI = 0;
    ...
    %i_0_SEI = i_0_SEI_tune*1.5e-06;
    i_0_SEI = i_0_SEI_tune*2e-03;
    %i_0_SEI = i_0_SEI_tune*3.0e-03;
    %i_0_SEI = i_0_SEI_tune*12e-3 ;%"SEI Exchange Current [A/m^2]"
    %OCP_SEI = 0 ; %SEI OCP [V]
    %OCP_SEI = 0.4 ; %SEI OCP [V]
    %OCP_SEI = 0.1 ; %SEI OCP [V]
    %OCP_SEI = 0.1 ; %SEI OCP [V]
    OCP_SEI = 0 ;
    %OCP_SEI = 0.1;
    alpha_SEI = 0.5 ;%"Charge transfer coefficient, SEI"
    %alpha_SEI = 0.3 ;%"Charge transfer coefficient, SEI"
    ...
    %k_SEI = k_0_SEI_tune*5.0e-11; %"Rate constant adjusted, SEI [m/s]"
    %k_SEI = k_0_SEI_tune*3.0e-11; %"Rate constant adjusted, SEI [m/s]"
    c_sol_0 = 4541; % Concentration of Solvent in bulk electrolyte [mol/m^3]
    %c_sol_0 = 3000; % Concentration of Solvent in bulk electrolyte [mol/m^3]
    ...
    k_li_tuning = 1.28;
    %k_li = k_li_tuning*9.0e-10; % Lithium plating rate constant
    ...
    %OCP_li =0.158; % OCP Plating Potential
    %OCP_li =0.1598; % OCP Plating Potential
...
if param == 2
    ...
    L_pos = 6.7e-05;             % [m]
    %}
    L_neg = 120e-6;%75e-6; % [m]
    L_sep = 30e-6;%17e-6; % [m] 
    L_pos = 160e-6;%40e-6; % [m]
