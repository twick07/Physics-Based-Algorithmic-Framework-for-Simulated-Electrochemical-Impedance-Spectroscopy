function [V_cell,Flag_convergence,time_vector,C_e,C_s_full,C_se,c_SEI,c_li_pl,I_app,psi_s,psi_e,j_tot,eps_sol,L_film,R_film,j_SEI,eps_sol_store] = P2D_function_general(Model_Parameters,N_tot,N_shell,order,param,time_step,sim_type,V_cut_off_low,V_cut_off_high,Cut_off_capacity,Ageing_models,C_e_init,C_s_full_init,C_se_init,c_SEI_init,c_li_init,I_app_init,C_rate,psi_s_init,psi_e_init,j_tot_init,eps_sol_init)
    ...
    function [dpsi_deltadt] = diff_psi_delta_dt_calculator(t,psi_delta,OCP_temp,alpha,F,R_const,T,ex_current,kappa,c_e,I_app,sigma,del_mid,del_x,t_li,As,Cdl_temp,boundary_conds,SEI_Settings)
        ...
        if SEI_con > 0 
            ...
            j_sum = (2.*ex_current).*sinh((alpha.*F./(R_const.*T)).*(psi_delta - OCP_temp- F*R_film_func.*j_prev_func));
        end
    ...
    %{
    D_sep_e_eff = (eps_sep_elec^brugg).*D_elec;
    D_neg_e_eff = (eps_neg_elec^brugg).*D_elec;
    D_pos_e_eff = (eps_pos_elec^brugg).*D_elec;
    %}
    ...
    eta_SEI_debug = zeros(time_max+1,N_tot_active);
    ...
    if sum(c_li_init) == 0
        c_li_pl = zeros(time_max,N_tot_active);
    else
        c_li_pl = zeros(time_max,N_tot_active);
    ...
    for i = 1:time_max
       ...
       if abs(I_app(i)) >= tol_I_app
            ...
            while norm(err(3),2) > tol
                ...
                eta_li_temp = zeros(3,N_tot_active);
                ...
                i_e_temp_iter = zeros(3,N_tot_active+1);
                diff_i_e_temp =  zeros(3,N_tot_active);
                ...
                %Calculate psi_s
                psi_s_temp(:,1) = eta_temp(:,1) + psi_e_temp(:,1) + OCP(i,1) + F*R_film(i,1)*j_ave_neg;
                ...
                diff_psi_s_m_temp(:,1) = -I_app(i)/(sigma(1)) ;
                ...
                for j = 1:N_neg-1
                    ...
                    % Calculate i_e for j+1 element
                    diff_i_e_temp(:,j) = F.*As(j).*j_tot_temp(:,j);

                    i_e_temp_iter(:,j+1) = i_e_temp_iter(:,j) + del(j).*diff_i_e_temp(:,j);
    
                    % Determine j_flux for j+1 element
                    eta_temp(:,j+1) = psi_s_temp(:,j+1) - psi_e_temp(:,j+1) - OCP(i,j+1) - F*R_film(i,j+1)*j_ave_neg;
                ...
                % Calculate i_e for the last element
               
                diff_i_e_temp(:,N_neg) = F.*As(N_neg).*j_tot_temp(:,N_neg);

                i_e_temp_iter(:,N_neg+1) = i_e_temp_iter(:,N_neg) + del(N_neg).*diff_i_e_temp(:,N_neg);
                ...
                elseif err_con == 2
                    ...
                    err = I_app(i) - i_e_temp_iter(:,N_neg+1);
                end
            ...
            eta_SEI_debug(i+1,:) = eta_SEI_temp(3,:);
       ...
       if abs(I_app(i)) < tol_I_app
           ...
           %% psi_delta_calculator for negative electrode
            ...
            SEI_Settings(1,1) = SEI*(I_app(i)<-tol_I_app_ageing);
           ...
           %% Calculate Fluxes
            if SEI == 0
                ...
                j_SEI(i+1,:) = -(I_app(i)<-tol_I_app_ageing).*( (i_o_SEI./F).*exp( -(alpha_SEI*F./(R_const.*T(i,:))).*eta_SEI  ) );
    ...
    j_SEI = j_SEI(1:stop_time+1,:);
    L_SEI = L_SEI(1:stop_time+1,:);
    L_film = L_film(1:stop_time+1,:);
    R_SEI = R_SEI(1:stop_time+1,:);
    R_film = R_film(1:stop_time+1,:);
    ...
    %end
    if and(sum(j_tot_init) ==0,sim_type ~=4)
        I_app_t_0 = 0;
    ...
    if sim_type == 4
        ...
        time_vector = time_vector(1:stop_time);
    end
