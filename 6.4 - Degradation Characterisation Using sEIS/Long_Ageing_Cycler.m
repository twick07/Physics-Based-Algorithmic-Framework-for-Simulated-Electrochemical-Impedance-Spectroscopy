...
tol_CV = 5e-4;
N_cycles = 50; % Should be a multiple of 5
test_freq = 1;
Test_cycles = linspace(1,N_cycles,(N_cycles/test_freq));
n_parameters = 46;
C_rate_non_test = 1;
C_rate_test = 1;
%% Model Selection
...
for k =1:N_cycles
    ...
    if sum(k == Test_cycles) == 0
        ...
        while abs(I_app_CV(end))>abs(I_app_cut_off)
            ...
            time_end = 1*time_step; % Note if increasing the time horizon, need to change the update of I_app_CV
            err = 1;
            err = ones(3,1);
            tol_con = tol_CV;
...
plot(time_vector_total,V_cell_total,'b');
ylabel('Voltage [V]');
legend('Applied Current [A/m^2]','Cell Voltage [V]');

figure(4);
plot(linspace(1,N_cycles,N_cycles),100*(1/(3600*I_app_total(2))).*q_lost,'b')
xlabel('Cycles [-]');
ylabel('Capacity Lost [%]')


Q_lost = 100*(1/(3600*I_app_total(2))).*q_lost;
Q_orig = (3600*I_app_total(2));
SoH_vector = (Q_orig - q_lost)/Q_orig ;

figure(5);
plot(linspace(1,N_cycles,N_cycles),100.*SoH_vector,'b')
xlabel('Cycles [-]');
...
Output_data = [time_vector_total' I_app_total' V_cell_total'];
Capacity_Lost_Vector = [cycle_end_points(1) Q_lost(1) ;cycle_end_points' Q_lost'];
SoH_Vector = [cycle_end_points(1) 100.*SoH_vector(1) ;cycle_end_points' 100.*SoH_vector'];

if SEI == 1
    
    csvwrite('50_cycle_KL_ageing_cap_lost.csv',Capacity_Lost_Vector);
    csvwrite('50_cycle_KL_ageing_SoH_lost.csv',SoH_Vector);
    csvwrite('Matlab_EIS_Frequencies_KL.csv',Freq_Impedance');
    csvwrite('Matlab_EIS_Real_Impedance_KL.csv',Real_Impedance');
    csvwrite('Matlab_EIS_Imag_Impedance_KL.csv',Imag_Impedance');
    csvwrite('Matlab_EIS_Impedance_KL.csv',Impedance_plots');
    csvwrite('Cycling_Data_KL_50_Cyles.csv',Output_data);

end

if SEI == 2

    csvwrite('50_cycle_DL_ageing_cap_lost.csv',Capacity_Lost_Vector);
    csvwrite('50_cycle_DL_ageing_SoH_lost.csv',SoH_Vector);
    csvwrite('Matlab_EIS_Frequencies_DL.csv',Freq_Impedance');
    csvwrite('Matlab_EIS_Real_Impedance_DL.csv',Real_Impedance');
    csvwrite('Matlab_EIS_Imag_Impedance_DL.csv',Imag_Impedance');
    csvwrite('Matlab_EIS_Impedance_DL.csv',Impedance_plots');
    csvwrite('Cycling_Data_DL_50_Cyles.csv',Output_data);
