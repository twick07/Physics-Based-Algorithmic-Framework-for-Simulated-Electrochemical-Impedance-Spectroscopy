function [V_cell,Flag_convergence,time_vector,C_e,C_s_full,C_se,c_SEI,c_li_pl,I_app,psi_s,psi_e,j_tot,eps_sol_store,eps_sol] = P2D_function_delta_t(Model_Parameters,N_tot,N_shell,order,param,V_cut_off_low,V_cut_off_high,Ageing_models,time_step,time_end,C_e_init,C_s_full_init,C_se_init,c_SEI_init,c_li_init,I_app_init,psi_s_init,psi_e_init,j_tot_init,eps_sol_init)

    %% Psi_Delta_function
    function [dpsi_deltadt] = diff_psi_delta_dt_calculator(t,psi_delta,OCP_temp,alpha,F,R_const,T,ex_current,kappa,c_e,I_app,sigma,del_mid,del_x,t_li,As,Cdl_temp,boundary_conds)
        
        psi_delta = psi_delta';

        j_sum = (2.*ex_current).*sinh((alpha.*F./(R_const.*T)).*(psi_delta - OCP_temp));

        %diffx_C_e_log = diff(log(c_e),1,2) ;
        ...
        diffx_C_e_log_x = [C_e_log_lbc diff(log(c_e),1,2)./del_mid(2:end-1) C_e_log_rbc];
        ...
        %i_e_lbc = boundary_conds(2,1);

        %i_e_rbc = boundary_conds(2,2);

        %diff_psi_delta_x = diff([psi_delta_lbc psi_delta psi_delta_rbc])./del_mid;
        diff_psi_delta_x = [psi_delta_lbc diff(psi_delta)./del_mid(2:end-1) psi_delta_rbc];

        %i_e_temp = cond.*(diff_psi_delta_x + kappa_d.*diffx_C_e_log + I_app./sigma);
        i_e_temp = cond.*(diff_psi_delta_x + (kappa_d./kappa).*diffx_C_e_log_x + I_app./sigma);

        dpsi_deltadt = ((1./(As.*Cdl_temp)).*(diff(i_e_temp,1,2)./del_x - As.*F.*j_sum))';

        %( (1./(As(1:N_neg).*Cdl(1:N_neg))).*( ([0; diff(conductivity_n.*,1) ;I_app])  -(2.*F.*ex_current(i,:))'.*sinh((alpha.*F./(R_const.*T(i,:))).*(psi_delta_temp_n' + OCP(i,:)))  ) )'
    end
    ...
    time_max = round(time_end./time_step);
    I_app = I_app_init*ones(1,time_max);
    ...
    eps_sol_store = zeros(time_max+1,N_tot_active);
    eps_sol_store(1,:) = eps_elec;
    ...
    if sum(sum(squeeze(C_s_full_init))) ==0
        C_s_full = zeros(time_max+1,N_shell,N_tot_active);
    ...
    diff_psi_s_m = zeros(time_max+1,N_tot_active+1);
    diff_psi_e_m = zeros(time_max+1,N_tot_active+1);
    diff_t_psi_s_m = zeros(time_max+1,N_tot_active);
    diff_t_psi_e_m = zeros(time_max+1,N_tot_active);
    diff_psi_delta_m = zeros(time_max+1,N_tot_active);
    time_vector = zeros(1,time_max+1);
    ...
    OCP_li = Model_Parameters(46); % OCP_li is li vs li+ which is fixed to 0
    ...
    for i = 1:time_max
        ...
        % Condition for checking if in first time step and adapting
        % transfer variables
        %Set initial values
       ...
       if abs(I_app(i)) >= tol_I_app
            ...
            while norm(err(3),2) > tol
                ...
                %% Create Guesses and Temporary Data Containers
                j1 = beta1*j_ave_neg;
                j2 = beta2*j_ave_neg;
                ...
                j_SEI_temp = zeros(3,N_tot_active);
                j_dl_temp = zeros(3,N_tot_active);
                j_li_temp = zeros(3,N_tot_active);
                ...
                eta_SEI_temp = zeros(3,N_tot_active);
                ...
                %% Set Boundary Conditions and Initial Guesses
                ...
                %Calculate psi_s
                psi_s_temp(:,1) = eta_temp(:,1) + psi_e_temp(:,1) + OCP(i,1) + F*R_film(i,1)*j_tot(i,1);
    
                %Calculate psi_delta
                psi_delta_temp(:,1) = psi_s_temp(:,1) - psi_e_temp(:,1);
                
                % Calculate double layer flux
                j_dl_temp(:,1) = (Cdl(1)/F)*((psi_delta_temp(:,1) - psi_delta(i,1))./time_step);
                
                
                % Calculate SEI flux
                if SEI > 0
                    eta_SEI_temp(:,1) = psi_s_temp(:,1) - psi_e_temp(:,1) - OCP_SEI - F*R_film(i,1).*j_ave_neg;
                    
                    j_SEI_temp(:,1) = -(I_app(i)<0).*( (i_o_SEI(1)./F).*exp( -(alpha_SEI*F./(R_const.*T(i,1))).*eta_SEI_temp(:,1)  ) );
                end

                if SEI  == 2
                    j_SEI_temp(:,1) = -(I_app(i)<-tol_I_app_ageing).*(c_sol_0./( (1./(k_SEI.*exp(- (alpha_SEI*F./(R_const.*T(i,1))).*eta_SEI_temp(:,1)))) + (L_SEI(i,1)./D_SEI) ) );
                end
                
                
                % Calculate Li Plating
                if li_pl > 0 
                    
                    %eta_li_temp(:,1) = psi_delta_temp(:,1)- OCP_li - F*R_film(i,1).*j_ave_neg;
                    eta_li_temp(:,1) = psi_delta_temp(:,1)- OCP_li;

                end

                if li_pl == 1
                    
                    j_li_temp(:,1) = -(eta_li_temp(:,1)<0).*(i_o_li(i,1)/F).*exp( -(alpha_pl_pos*F./(R_const.*T(i,1))).*eta_li_temp(:,1)  );

                end

                if li_pl == 2

                    j_li_temp(:,1) = (eta_li_temp(:,1)<0).*(i_o_li(i,1)/F).*( exp( ((alpha_pl_neg*F)./(R_const.*T(i,1))).*(eta_li_temp(:,1)) ) - exp( ((-alpha_pl_pos*F)./(R_const*T(i,1))).*(eta_li_temp(:,1)))) ;

                % Caluclate Li Stripping

                    j_li_temp(:,1) = j_li_temp(:,1) + (eta_li_temp(:,1)>0).*(c_li_pl(i,1)./(1+c_li_pl(i,1))).*(i_o_li(i,1)/F).*( exp( ((alpha_pl_neg*F)./(R_const.*T(i,1))).*(eta_li_temp(:,1)) ) - exp( ((-alpha_pl_pos*F)./(R_const*T(i,1))).*(eta_li_temp(:,1)))) ;
                end

                % Calculate total flux
                j_tot_temp(:,1) = j_flux_temp(:,1) + j_dl_temp(:,1) + j_SEI_temp(:,1) + j_li_temp(:,1);
                ...
                %% Loop for solving negative domain equations
                for j = 1:N_neg-1
                    ...
                    % Determine j_flux for j+1 element
                    eta_temp(:,j+1) = psi_s_temp(:,j+1) - psi_e_temp(:,j+1) - OCP(i,j+1) - F*R_film(i,j+1)*j_tot(i,j+1);
    
                    j_flux_temp(:,j+1) = 2.*ex_current(i,j+1).*sinh((alpha*F/(R_const*T(i,j+1)))*eta_temp(:,j+1)) ;

                    %Calculate psi_delta for j+1 element
                    psi_delta_temp(:,j+1) = psi_s_temp(:,j+1) - psi_e_temp(:,j+1);

                    % Calculate double layer flux for j+1 element
                    j_dl_temp(:,j+1) = (Cdl(j+1)/F)*((psi_delta_temp(:,j+1)- psi_delta(i,j+1))./time_step);
                    

                    % Calculate SEI flux for j+1 element
                    if SEI > 0
                        eta_SEI_temp(:,j+1) = psi_s_temp(:,j+1) - psi_e_temp(:,j+1) - OCP_SEI - F*R_film(i,j+1).*j_ave_neg;
                    
                        j_SEI_temp(:,j+1) = -(I_app(i)<-tol_I_app_ageing).*( (i_o_SEI(j+1)./F).*exp( -(alpha_SEI*F./(R_const.*T(i,j+1))).*eta_SEI_temp(:,j+1)  ) );
                    end

                    if SEI  == 2

                        j_SEI_temp(:,j+1) = -(I_app(i)<0).*(c_sol_0./( (1./(k_SEI.*exp(- (alpha_SEI*F./(R_const.*T(i,j+1))).*eta_SEI_temp(:,j+1)))) + (L_SEI(i,j+1)./D_SEI) ) );
                    end
                    if li_pl > 0
                        % Calculate Plating overpotential
                        %eta_li_temp(:,j+1) = psi_delta_temp(:,j+1)- OCP_li - F*R_film(i,j+1).*j_ave_neg;
                        eta_li_temp(:,j+1) = psi_delta_temp(:,j+1)- OCP_li;
                    end

                    if li_pl == 1
                    
                        j_li_temp(:,j+1) = -(eta_li_temp(:,j+1)<0).*(i_o_li(i,j+1)/F).*exp( -(alpha_pl_pos*F./(R_const.*T(i,j+1))).*eta_li_temp(:,j+1)  );

                    end

                    if li_pl == 2
                        % Calculate Li Plating
                        j_li_temp(:,j+1) = (eta_li_temp(:,j+1)<0).*(i_o_li(i,j+1)/F).*( exp( ((alpha_pl_neg*F)./(R_const.*T(i,j+1))).*(eta_li_temp(:,j+1)) ) - exp( ((-alpha_pl_pos*F)./(R_const*T(i,j+1))).*(eta_li_temp(:,j+1)))) ;
        
                        % Caluclate Li Stripping
        
                        j_li_temp(:,j+1) = j_li_temp(:,j+1) + (eta_li_temp(:,j+1)>0).*(c_li_pl(i,j+1)./(1+c_li_pl(i,j+1))).*(i_o_li(i,j+1)/F).*( exp( ((alpha_pl_neg*F)./(R_const.*T(i,j+1))).*(eta_li_temp(:,j+1)) ) - exp( ((-alpha_pl_pos*F)./(R_const*T(i,j+1))).*(eta_li_temp(:,j+1)))) ;
                    end

                    % Calculate total flux
                    j_tot_temp(:,j+1) = j_flux_temp(:,j+1) + j_dl_temp(:,j+1) + j_SEI_temp(:,j+1) + j_li_temp(:,j+1);
            ...
            % Update Fluxes
            j_tot(i+1,:) = j_tot_temp(3,:);
            j_flux(i+1,:) = j_flux_temp(3,:);
            j_dl(i+1,:) = j_dl_temp(3,:);
            j_SEI(i+1,:) = j_SEI_temp(3,:);
            j_li(i+1,:) = j_li_temp(3,:);
            ...
            if sum(isinf(j_flux(i+1,1:N_neg))) > 0
                Flag_convergence = 1;
                break;
    
            end
    
            if sum(isnan(j_flux(i+1,1:N_neg))) > 0
                Flag_convergence = 1;
                break;
            ...
            if isreal(j_flux(i+1,1:N_neg)) == 0
                Flag_convergence = 1;
                break;
           ...
           for j = 1:N_neg+N_sep
               ...
               %Calculate psi_e for j+1 element
                ...
                diff_psi_e_m(i+1,j+1) = (1/kappa_k_plus_half)*((kappa_k_minus_half)*diff_psi_e_m(i+1,j) + (kd_k_plus_half*diffx_C_e_log(j+1) - kd_k_minus_half*diffx_C_e_log(j)) - (F*As(j)*del(j)*j_tot(i,j)));
           ...
           while norm(err(3),2) > tol
               ...
               % Create Guesses
                ...
                j_flux_temp = ones(3,N_tot_active)*diag(j_flux(i+1,:));
                j_tot_temp = ones(3,N_tot_active)*diag(j_tot(i+1,:));
                j_dl_temp = ones(3,N_tot_active)*diag(j_dl(i+1,:));
                psi_e_temp = ones(3,N_tot_active)*diag(psi_e(i+1,:));
                %psi_s_temp = zeros(3,N_tot_active);
                eta_temp = zeros(3,N_tot_active);
                %diff_psi_s_m_temp = zeros(3,N_tot_active+1);
                diff_psi_e_m_temp = ones(3,N_tot_active+1)*diag(diff_psi_e_m(i+1,:));
                ...
                %Calculate psi_s
                psi_s_temp(:,N_neg+N_sep+1) = eta_temp(:,N_neg+N_sep+1) + psi_e_temp(:,N_neg+N_sep+1) + OCP(i,N_neg+N_sep+1);
                
                %Calculate psi_delta
                psi_delta_temp(:,N_neg+N_sep+1) = psi_s_temp(:,N_neg+N_sep+1) - psi_e_temp(:,N_neg+N_sep+1);
                
                % Calculate double layer flux
                j_dl_temp(:,N_neg+N_sep+1) = (Cdl(N_neg+N_sep+1)/F)*((psi_delta_temp(:,N_neg+N_sep+1)- psi_delta(i,N_neg+N_sep+1))./time_step);
                ...
                for j = N_neg+N_sep+1:N_tot_active-1
                    ...
                    eta_temp(:,j+1) = psi_s_temp(:,j+1) - psi_e_temp(:,j+1) - OCP(i,j+1);
           ...
           j_flux(i+1,:) = j_flux_temp(3,:);
           j_dl(i+1,:) = j_dl_temp(3,:);
           diff_psi_e_m(i+1,:) = diff_psi_e_m_temp(3,:);
           diff_psi_s_m(i+1,:) = diff_psi_s_m_temp(3,:);

           psi_s(i+1,:) = psi_s_temp(3,:);
           psi_e(i+1,:) = psi_e_temp(3,:);
           psi_delta(i+1,:) = psi_s(i+1,:) - psi_e(i+1,:);
           eta(i+1,:) = psi_delta(i+1) - OCP(i,:);
            ...
            if sum(isinf(j_flux(i+1,:))) > 0
                Flag_convergence = 2;
                break;
    
            end
    
            if sum(isnan(j_flux(i+1,:))) > 0
                Flag_convergence = 2;
                break;
       ...
       %% Calculate SEI growth
        ...
        if coupling == 2
            ...
            D_e_eff(1:N_neg) = (eps_elec(1:N_neg).^brugg(1:N_neg)).*D_elec;
            D_e = D_e_eff;
            %As(1:N_neg) = 3.*eps_sol(1:N_neg)./Rs(1:N_neg);

            %sigma(1:N_neg) = sigma_neg.*(eps_sol(1:N_neg).^brugg_neg);
       ...
       if order == 4
           ...
           % Add j_flux effect
           C_s_shells_next(end,:) = C_s_shells_next(end,:) - time_step*j_flux(i+1,:).*[(Sa_neg(end)/dV_neg(end))*ones(1,N_neg) zeros(1,N_sep) (Sa_pos(end)/dV_pos(end))*ones(1,N_pos)];
       ...
       if order == 6
        ...
        for k = 1 : N_tot_active
            ...
            if k <N_neg+1
                ...
                op = odeset('reltol',tol);
                [t,C_s_shell_temp] = ode15s(@(t,C_s_shell_temp) (D_s(k)./dV_neg').*[Sa_neg(1)*diff(C_s_shell_temp(1:2))./dR_neg ;diff(Sa_neg(1:end-1)'.*diff(C_s_shell_temp)./dR_neg) ; -Sa_neg(end)*j_flux(i+1,k)./D_s(k) - Sa_neg(end-1)*diff(C_s_shell_temp(end-1:end))./dR_neg ],[0 time_step], C_s_shell',op);
            ...
            if k>N_neg+N_sep
                [t,C_s_shell_temp] = ode15s(@(t,C_s_shell_temp) (D_s(k)./dV_pos').*[Sa_pos(1)*diff(C_s_shell_temp(1:2))./dR_pos ;diff(Sa_pos(1:end-1)'.*diff(C_s_shell_temp)./dR_pos) ; -Sa_pos(end)*j_flux(i+1,k)./D_s(k) - Sa_pos(end-1)*diff(C_s_shell_temp(end-1:end))./dR_pos ],[0 time_step], C_s_shell',op);
       ...
       if order == 7
               ...
               diff_C_s_ave_change = ((-3./Rs).*j_flux(i+1,:));
               ...
               diff_C_se_change = ( C_s_ave(i+1,:) - ( (Rs./(5*D_s)).*j_flux(i+1,:)) );
    ...
    eps_sol_store = eps_sol_store(1:stop_time+1,:);
    %end
    if sum(j_tot_init) ==0
        I_app_t_0 = 0;
