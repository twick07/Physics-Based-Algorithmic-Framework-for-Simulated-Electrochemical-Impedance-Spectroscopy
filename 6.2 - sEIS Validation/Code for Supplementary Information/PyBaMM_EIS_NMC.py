# %%
# Simple 1C Discharge Simulation Using Prebuilt Parameter Set
...
# %%
# Build P2D_Model_2 - The pybamm built in C-P2D-DFN model
...
param_dict = {'Thermodynamic factor': Model_Parameters[28],
             ...
             'Separator Bruggeman coefficient (electrode)': Model_Parameters[1],
              ...
              #'Electrolyte diffusivity [m2.s-1]': D_e_function,
...
frequencies = np.logspace(-3, 3, No_of_frequencies)
...
df.to_csv('NMC_EIS_time_vectors_'+SoC_text+'.csv')

#load output data into a DataFrame object:
df = pd.DataFrame(output_data_current)

# writing output data frame to a CSV file
df.to_csv('NMC_EIS_current_vectors_'+SoC_text+'.csv')

#load output data into a DataFrame object:
df = pd.DataFrame(output_data_voltage)

# writing output data frame to a CSV file
df.to_csv('NMC_EIS_voltage_vectors_'+SoC_text+'.csv')

#load output data into a DataFrame object:
df = pd.DataFrame(output_data_impedance)

# writing output data frame to a CSV file
df.to_csv('NMC_EIS_Impedance_'+SoC_text+'.csv')
