function [V_cell,Flag_convergence,time_vector,C_e,C_s_full,C_se,c_SEI,c_li_pl,I_app,psi_s,psi_e,j_tot] = P2D_MDN_function_ODE_int(Model_Parameters,N_tot,N_shell,order,param,time_step,sim_type,V_cut_off_low,V_cut_off_high,Cut_off_capacity,Ageing_models,C_e_init,C_s_full_init,C_se_init,c_SEI_init,c_li_init,I_app_init,C_rate,psi_s_init,psi_e_init,j_tot_init)
    ...
    j_li = zeros(time_max,N_tot_active);
    ...
    for i = 1:time_max
        ...
        if i == 1
            ...
            yp0 = ((psi_delta(i,1:N_neg)-psi_delta(i-1,1:N_neg))./time_step);

        end

        [t,psi_delta_temp_n] = ode15i(@(t,psi_delta_temp_n)  diff_psi_delta_dt_calculator(t,psi_delta_temp_n,OCP(i,1:N_neg),alpha,F,R_const,T_amb,ex_current(i,1:N_neg),kappa_n_extended,C_e_reduced(1:N_neg),I_app(i),sigma(1),del_mid(1:N_neg+1),del(1:N_neg),t_li,As(1:N_neg),Cdl(1:N_neg),boundary_conds), [0 time_step], psi_delta(i,1:N_neg)',yp0, op);
        %}
        ...
        if i == 1
            ...
            yp0 = ((psi_delta(i,N_neg+N_sep+1:N_tot)-psi_delta(i-1,N_neg+N_sep+1:N_tot))./time_step);

        end
        
        [t,psi_delta_temp_p] = ode15i(@(t,psi_delta_temp_p)  diff_psi_delta_dt_calculator(t,psi_delta_temp_p,OCP(i,N_neg+N_sep+1:N_tot),alpha,F,R_const,T_amb,ex_current(i,N_neg+N_sep+1:N_tot),kappa_p_extended,C_e_reduced(N_neg+N_sep+1:N_tot),I_app(i),sigma(end),del_mid(1:N_pos+1),del(N_neg+N_sep+1:N_tot),t_li,As(N_neg+N_sep+1:N_tot),Cdl(N_neg+N_sep+1:N_tot),boundary_conds), [0 time_step], psi_delta(i,N_neg+N_sep+1:N_tot)',yp0, op);
        %}
