...
%% Import PyBaMM Data
Impedance_data = xlsread("LCO_EIS_Impedance_"+int2str(SoC)+".csv");
Time_data = xlsread("LCO_EIS_time_vectors_"+int2str(SoC)+".csv");
Current_data = xlsread("LCO_EIS_current_vectors_"+int2str(SoC)+".csv");
Voltage_data = xlsread("LCO_EIS_voltage_vectors_"+int2str(SoC)+".csv");
...
if SoC == 100
    ...
    print('sEIS_Voltage_Current_f_1mHz', '-dpng', ['-r', num2str(dpi)]);
    figure(output_voltage_current_plots(2));
    print('sEIS_Voltage_Current_f_1Hz', '-dpng', ['-r', num2str(dpi)]);
    figure(output_voltage_current_plots(3));
    print('sEIS_Voltage_Current_f_100Hz', '-dpng', ['-r', num2str(dpi)]);

end

figure(i+1);
print("sEIS SoC "+int2str(SoC)+" LCO", '-dpng', ['-r', num2str(dpi)]);
...
csvwrite("Matlab_LCO_EIS_"+int2str(SoC)+"_SoC.csv",Export_matrix);
csvwrite("PyBaMM_LCO_EIS_"+int2str(SoC)+"_SoC.csv",Export_matrix_pybamm);
